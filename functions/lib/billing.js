...
exports.calculateBilling = exports.getBillingPeriod = exports.formatDateUTC = exports.addMonthsUTC = exports.parseDateUTC = void 0;
...
function getBillingPeriod(intakeDateStr, periodIndex) {
 ...
 */
function calculateBilling(member, sponsorships, // Active sponsorships
asOfDateStr) {
    // const intakeDate = parseDateUTC(member.intakeDate); // Unused
    ...
    let totalCovered = 0;
    const newCharges = [];
    const newSponsorshipCharges = [];
    // Clone sponsorships to track remainingAmount depletion in memory
    const mutableSponsorships = sortedSponsorships.map(s => (Object.assign({}, s)));
    ...
    while (cycles < MAX_CYCLES) {
        ...
        const bedCharge = {
            id: `charge_${member.id}_${currentPeriodIndex}`,
            memberId: member.id,
        ...
        for (const sponsor of mutableSponsorships) {
            ...
            if (sponsorStart > periodStart)
                continue; // Starts later
            if (sponsorEnd && sponsorEnd <= periodStart)
                continue; // Ended before this period
