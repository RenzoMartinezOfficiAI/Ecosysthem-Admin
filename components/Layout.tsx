...
const Layout: React.FC<LayoutProps> = ({ children, currentRole, currentPage, onNavigate }) => {
  ...
  if (role === UserRole.ADMIN || role === UserRole.OPERATIONS_MANAGER || role === UserRole.HOUSE_LEAD) {
      menuItems.push(
    ...
    <div className="flex h-screen bg-matte-950 overflow-hidden font-sans">
      ...
      >
        ...
        <div className="p-4 border-t border-white/5 bg-matte-950/30">
          ...
          <button 
            onClick={signOut}
            className={`btn w-full text-xs py-2 text-[#ff4d4d] border border-[#ff4d4d] hover:bg-[#ff4d4d]/10 transition-all whitespace-nowrap overflow-hidden flex items-center justify-center gap-2`}
          >
